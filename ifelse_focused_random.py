...
if __name__ == "__main__":
	...
	con = sqlite3.connect('ifelse.sqlite')
	cur = con.cursor()
	...
	for COUNTER in range(int(1e6)):
		ql = qiling.Qiling(["ifelse"], ".", console=console, stdin=StringBuffer(), stdout=StringBuffer())

		ql.hook_code(print_asm)

		## Pick a random address of the main execution trace 
		fi_addr = random.choice(MAIN_ADDRS)
		fi_random_bytes = os.urandom(4)
		...
		disasm = ''
		for i in md.disasm(fi_random_bytes, fi_addr):
		...
		cur.execute(f'''INSERT INTO {table}(id, data, category, random, disasm, exception, trace) VALUES(?, ?, ?, ?, ?, ?, ?)''', 
			(COUNTER, data, category, fi_random_bytes.hex(), disasm, exception, '\n'.join(trace)))
			# (COUNTER, data.replace(b'\n', b'').decode(), fi_random_bytes, disasm, exception, trace))

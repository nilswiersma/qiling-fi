...
from fault_models import random_word, flip_single_bit
...
def main():
    ...
    t = tproduct_builder(range(int(1e5)), [IF_ADDR])
    for _, addr in tproduct(t, range(int(1e5)), [IF_ADDR]):
        exception = ''
        COUNTER += 1    
        ...
        ql = QilingFi(
            ...
            # QilingFi args
            trace_start=IF_ADDR, trace_end=IF_ADDR+4,
        ...
        if terminal_logging:
            tqdm.write(f'{addr:x} | {ins[::-1].hex()} | {disasm} | {data} | {exception}')
            # tqdm.write(f'FREE_BEER: {FREE_BEER} | NO_BEER: {NO_BEER} | EXCEPTION: {EXCEPTION}')
            t.set_description(f'FREE_BEER: {FREE_BEER} | NO_BEER: {NO_BEER} | EXCEPTION: {EXCEPTION}')
